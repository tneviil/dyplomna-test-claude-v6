...
(() => {
  ...
  $$('[data-xp]').forEach((root) => {
    ...
    let auto = !mq.matches && N > 1;
    let timer = null;
    let inView = false;
    let hover = false;
    let focus = false;
    const halt = () => { if (timer) { clearInterval(timer); timer = null; } };
    const run = () => { if (!timer && auto && inView && !hover && !focus && !document.hidden) timer = setInterval(() => go(cur + 1), DELAY); };
    const setAuto = (on) => {
      auto = on && N > 1;
    ...
    root.addEventListener('mouseenter', () => { hover = true; halt(); });
    root.addEventListener('mouseleave', () => { hover = false; run(); });
    root.addEventListener('focusin', () => { focus = true; halt(); });
    root.addEventListener('focusout', (e) => { if (!root.contains(e.relatedTarget)) { focus = false; run(); } });
    if ('IntersectionObserver' in window) {
      new IntersectionObserver((en) => { inView = en[0].isIntersecting; if (inView) run(); else halt(); }, { threshold: .25 }).observe(root);
  ...
  (() => {
    ...
    $$('[data-rv]').forEach((rv) => {
      ...
      let manual = false; // once the visitor pages by hand the automatic rotation stays off
      const DELAY = 3000;
      ...
      const run = () => {
        ...
        if (!inView || hover || focus || document.hidden) return;
      ...
      rv.addEventListener('mouseenter', () => { hover = true; halt(); });
      rv.addEventListener('mouseleave', () => { hover = false; run(); });
      rv.addEventListener('focusin', () => { focus = true; halt(); });
